...
let scene: any, camera: any, renderer: any, cube: any, axes: any, sphere: any, torus: any, planet: any,
  planetGround: any, light: any,
  cube1: any, cube2: any, cubeDepth: any, sphereDepth: any, cylinder: any, cubeLightSensitive: any;
let ADD = 0.05;
let donuts: any[] = [];
let rings: any[] = [];
...
let lightSensitive = function () {
  let geometry = new THREE.BoxGeometry(3, 3, 3);
  let material = new THREE.MeshLambertMaterial({
    color: 0x7fc5f9,
    side: THREE.DoubleSide
  });
  cubeLightSensitive = new THREE.Mesh(geometry, material);
  scene.add(cubeLightSensitive);
...
let ambientLight = function () {
  ...
  cubeLightSensitive = new THREE.Mesh(geometry, material);
  scene.add(cubeLightSensitive);
  let geometryPlane = new THREE.PlaneGeometry(1000,1000,50,50);
  material = new THREE.MeshPhongMaterial({color: 0x693421, side: THREE.DoubleSide});
  planet = new THREE.Mesh(geometryPlane, material);
...
let mainLoop = function () {
  ...
  //   ADD *= -1;
  // }
  cubeLightSensitive.rotation.x += ADD;
  cubeLightSensitive.rotation.y += ADD;
  cubeLightSensitive.rotation.z += ADD;
  light.intensity += ADD;
  if(light.intensity > 8 || light.intensity < -3){
    ADD *= -1;
  }
